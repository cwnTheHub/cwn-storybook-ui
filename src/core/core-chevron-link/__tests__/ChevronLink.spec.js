...
describe("ChevronLink", () => {
  const doShallow = (overrides = {}) =>
    shallow(<ChevronLink {...overrides}>Go home</ChevronLink>);
  const doRender = (overrides = {}) =>
    render(<ChevronLink {...overrides}>Go home</ChevronLink>);
  ...
  /* it("forwards refs", () => {
    ...
    const target = link.find("StyledComponent").at(0).childAt(0).instance();
