...
describe("media", () => {
  ...
  describe("until()", () => {
    it("accepts a function", () => {
      ...
      expect(style).toEqual({
        [`@media (max-width: ${breakpoints.md - 1}px)`]: { color: "red" },
      });
    });
  ...
  it("can return css with no media query if from/until/and are all undefined", () => {
    const style = media.from(undefined).until(undefined).and(undefined).css({
      color: "red",
    });
  ...
  it("handles min-width queries", () => {
    let style = media.from("xs").until(undefined).css({
      color: "red",
    });

    expect(style).toEqual({ color: "red" });

    style = media.from("md").until(undefined).css({
      color: "red",
    });
