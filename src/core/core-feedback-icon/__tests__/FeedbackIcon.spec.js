...
describe("FeedbackIcon", () => {
  ...
  const doShallow = (props) =>
    shallow(<FeedbackIcon {...defaultProps} {...props} />);
  ...
  it("passes additional attributes to the element", () => {
    ...
    expect(feedbackIcon.props().id).toBe("the-id");
    expect(feedbackIcon.props()["data-some-attr"]).toBe("some value");
  });
  ...
  it("does not allow custom CSS", () => {
    ...
    expect(feedbackIcon.props()["className"]).not.toBe("my-custom-class");
    expect(feedbackIcon.props()["style"]).toBe(undefined);
  });
});
