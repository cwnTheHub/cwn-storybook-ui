...
describe("ToggleSwitch", () => {
  ...
  const doShallow = (props) =>
    shallow(<ToggleSwitch {...defaultProps} {...props} />);
  const doMount = (props) =>
    mount(<ToggleSwitch {...defaultProps} {...props} />);
  ...
  it("should have for attribute matching passed id", () => {
    ...
    expect(toggleSwitch.find("StyledLabel").prop("htmlFor")).toEqual(
      defaultProps.id
    );
  });
  ...
  it("should have label text with an id that matches the checkbox aria-labelledby attribute", () => {
    ...
    expect(
      toggleSwitch.find(`#${defaultProps.id}`).at(0).prop("aria-labelledby")
    ).toEqual(`${defaultProps.id}-label`);

    expect(
      toggleSwitch.find(`#${defaultProps.id}`).at(0).prop("aria-labelledby")
    ).toEqual(toggleSwitch.find(`#${defaultProps.id}-label`).at(0).prop("id"));
  });
  ...
  it("should be checked when checked property is true", () => {
    ...
    expect(
      toggleSwitch.find(`#${defaultProps.id}`).at(0).prop("aria-checked")
    ).toEqual(true);
  ...
  it("should call onFocus handler when provided ", () => {
    ...
    toggleSwitch.find(`#${defaultProps.id}`).at(0).simulate("focus", mockEvent);
  ...
  it("should call onBlur handler when provided ", () => {
    ...
    toggleSwitch.find(`#${defaultProps.id}`).at(0).simulate("blur", mockEvent);
