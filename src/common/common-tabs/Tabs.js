...
import { ChevronRight, ChevronLeft } from "../../core/core-interactive-icon";
...
const Tabs = (props) => {
  ...
  useEffect(() => {
    ...
    const tabIndex = props.children.findIndex(
      (child) => child.props.id === open
    );
    ...
    if (!tabScrollIntervals.current.length) {
      ...
      tabsArray.forEach((value, index) => {
        ...
        if (
          ...
          currentTabsLength + increment >
            tabContainerWidthWithBuffer - MARGIN_BUFFER &&
          tabScrollIntervals.current.length !== 0
        ) {
      ...
      if (tabScrollIntervals.current.length > 0) {
        ...
        for (let i = tabsArray.length - 1; i >= 0; i -= 1) {
          ...
          if (
            currentTabsLength + increment >
            tabContainerWidth - MARGIN_BUFFER
          ) {
  ...
  const setResizeScrollIntervals = () => {
    ...
    if (tabsTranslatePosition !== 0) {
      ...
      tabSumArr.unshift(0);
      const diffArr = tabSumArr.map((val) =>
        Math.abs(val - Math.abs(tabsTranslatePosition))
      );
  ...
  const scrollTabs = (direction) => {
    ...
    if (direction === "right") {
      if (isRightArrowVisible && !isLeftArrowVisible) {
        currentPosition -=
          tabScrollIntervals.current[tabScrollPosition.current];
      } else {
        currentPosition -=
          tabScrollIntervals.current[tabScrollPosition.current];
      }
    ...
    if (direction === "left") {
      if (tabScrollPosition.current === 1) {
        ...
        currentPosition +=
          tabScrollIntervals.current[tabScrollPosition.current - 1];
      }
      tabScrollPosition.current -= 1;
    }
    ...
    if (
      ...
      if (
        ...
        document.activeElement !==
          tabNavRef.current.node.parentNode.children[0].children[0]
      ) {
        currentFocus.current += 1;
        tabNavRef.current.node.parentNode.children[
    ...
    const handleGlobalTabUpEvent = (e) => {
      ...
      if (e.keyCode === TAB) {
        for (let i = 0; i < numTabs; i += 1) {
          if (
            document.activeElement ===
            tabNavRef.current.node.parentNode.children[i].children[0]
          ) {
