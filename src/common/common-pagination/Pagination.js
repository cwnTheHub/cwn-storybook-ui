...
class Pagination extends Component {
  ...
  mapTabs = (isMobile) => {
    ...
    return this.props.children.map((item, i) => {
      ...
      if (current === index) {
        return (
          <PaginationCurrent key={hash(`${i}-1`)}>
            {index}
            <A11yContent>
              {getCopy(copyDictionary, copy).currentText}
            </A11yContent>
          </PaginationCurrent>
        );
      }
      ...
      if (this.checkForEllipsis(index, isMobile)) {
        return (
          <PaginationEllipsis key={hash(`${i}-4`)}>...</PaginationEllipsis>
        );
      }
  ...
  render() {
    ...
    const previousLabel =
      copy !== "fr" ? "Go to previous panel" : "Aller au panneau précédent";
    const NextLabel =
      copy !== "fr" ? "Go to next panel" : "Aller au prochain panneau";
    return (
      <PaginationContainer {...safeRest(rest)}>
        ...
        <Controls>
          ...
          <NextPaginationContainer showNext={this.state.showNext}>
            ...
            >
              <ButtonLabel>{nextText}</ButtonLabel>
            </PaginationButtonStyle>
          </NextPaginationContainer>
        </Controls>
      </PaginationContainer>
