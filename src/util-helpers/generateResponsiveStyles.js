...
export const isMobileBreakpoint = (breakpoint) =>
  MOBILE_BREAKPOINTS.indexOf(breakpoint) !== -1;
export const isDesktopBreakpoint = (breakpoint) =>
  DESKTOP_BREAKPOINTS.indexOf(breakpoint) !== -1;

const isResponsiveProp = (prop) =>
  prop &&
  BREAKPOINTS.find((breakpoint) =>
    Object.prototype.hasOwnProperty.call(prop, breakpoint)
  );

const getResponsiveProps = (props) =>
  Object.keys(props).filter((prop) => isResponsiveProp(props[prop]));
...
export const generateStyles = (breakpoints, style) => {
  const styles = breakpoints.reduce((acc, breakpoint) => {
    ...
    if (
      !(
        typeof breakpoint.from === "undefined" &&
        typeof breakpoint.until === "undefined"
      )
    ) {
      const result = media
        ...
        .css(
          typeof style === "function"
            ? style(props, breakpoint.from, breakpoint.until)
            : style
        );
...
const handleBoundaryCrossing = (acc, curr) => {
  if (
    ...
    ((curr.until !== "md" && isDesktopBreakpoint(curr.until)) ||
      typeof curr.until === "undefined")
  ) {
